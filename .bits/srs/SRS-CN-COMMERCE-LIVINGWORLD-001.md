# SRS-CN-COMMERCE-LIVINGWORLD-001 — Commerce Guild · Living-World Floor

**Guild:** commerce · **Champion:** Alex · **Branch:** `bits/livingworld`

## Objective
The commerce guild is the outreach + growth core (leads, campaigns, partnerships). In the Citadel living world (the Aincrad Tower of Babel) this guild owns
...
1. **Emit floor events** on the guild NATS namespace `citadel.commerce.*` (already this repo's declared prefix):
   - `citadel.commerce.activity` — a heartbeat with an `activity_level` 0..1 (drives the floor's size + glow).
   - `citadel.commerce.quest` — a quest opened/closed (a real unit of guild work → an in-game quest marker).
   - `citadel.commerce.champion` — the champion's (Alex) state (idle/working/blocked).
2. **Expose a public realm feed** `GET /realm/commerce.json` — non-secret, non-PII shape the game reads:
   ```json
   {
     "guild": "commerce",
     "champion": "Alex",
   ...
   ```
3. **Bind the champion** — `Alex` is this floor's guildmaster avatar in `/game/party.json`.

## Acceptance
- The feed returns the shape above; `activity_level` moves with real guild activity (no fabricated numbers).
- The three `citadel.commerce.*` events publish on real state changes; a subscriber can render the floor from them.
